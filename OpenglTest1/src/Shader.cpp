...
//分析shader源码
ShaderProgramSource Shader::ParseShader(const std::string& filepath,bool tcs) {

    std::ifstream stream(filepath);

    enum class ShaderType {
        NONE = -1, VERTEX = 0, FRAGMENT = 1,TCS = 2,TES = 3
    };
    ...
    while (getline(stream, line)) {
        ...
        if (line.find("#shader") != std::string::npos)//a是否找到标记
        {
            ...
            else if (line.find("tessContral") != std::string::npos) {
                type = ShaderType::TCS;
            }
            else if (line.find("tessEva") != std::string::npos) {
                type = ShaderType::TES;
            }
        }
        else
...
unsigned int Shader::CreatShader(const std::string& vertexShader, const std::string& fragmentShader,const std::string& tessContralShader, const std::string& tessEvalShader)
{
    ...
    if (Tcs)
    {
        tcs = CompileShader(GL_TESS_CONTROL_SHADER, tessContralShader);
...
Shader::Shader(const std::string& filePath,bool tcs)
	:m_FilePath(filePath), m_RendererID(0),Tcs(tcs)
{
    ShaderProgramSource source = ParseShader(filePath, Tcs);
    m_RendererID = CreatShader(source.VertexSource, source.FragmentSource,source.TcsSource,source.TesSource);
    std::cout << "Vertex" << std::endl;
    std::cout << source.VertexSource << std::endl;
    std::cout << "Fragment" << std::endl;
    std::cout << source.FragmentSource << std::endl;
    std::cout << "Tcs" << std::endl;
    std::cout << source.TcsSource << std::endl;
    std::cout << "Tes" << std::endl;
    std::cout << source.TesSource << std::endl;
}
